...
ward_50 <- read.csv(file.path(OutputsDir, "NMEP Presentation Reprioritization Tables", 
                              "Yobe", paste0("yobe_scenario_3", ".csv"))) %>%  left_join(Yobe_ranks, by = c("WardCode"))
Yobe_shp$WardCode <- as.numeric(Yobe_shp$WardCode)
priorite_50 <- Yobe_shp  %>%  left_join(ward_50, by = c("WardCode")) %>%  filter(!is.na(WardPopulation)) %>% 
mutate(new_rank = rank(ranks))


p_50<- ggplot(data = filter(lGAshp, State == "Yobe")) +
  geom_sf(fill = "red") +
  geom_sf(data = priorite_50, aes(geometry = geometry), fill = "green") +
  # scale_fill_manual(values = c("Not Reprioritized" = "red", "Reprioritized" = "green"),
  #                   name = "Status") +
  geom_text_repel(data = filter(lGAshp, State == "Yobe"),
...
ward_75 <- read.csv(file.path(OutputsDir, "NMEP Presentation Reprioritization Tables", 
                              "Yobe", paste0("yobe_scenario_4", ".csv"))) %>%  left_join(Yobe_ranks, by = c("WardCode"))
Yobe_shp$WardCode <- as.numeric(Yobe_shp$WardCode)
priorite_75 <- Yobe_shp  %>%  left_join(ward_75, by = c("WardCode")) %>%  filter(!is.na(WardPopulation)) %>% 
  mutate(new_rank = rank(ranks))


p_75<- ggplot(data = filter(lGAshp, State == "Yobe")) +
  geom_sf(fill = "red") +
...
ggsave(paste0(FigDir,"/", "urban_50_75", "/", Sys.Date(),"_Yobe_reprioritization_scenarios.pdf"), p_high_urban, width = 13, height = 14)
