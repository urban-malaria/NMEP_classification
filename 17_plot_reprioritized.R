...
delta_ranks <- read.csv(file.path(OutputsDir, "rankings", "Delta_rankings.csv")) %>% 
  dplyr::select(WardName,WardCode, ranks)
...
prioritized_katsina3 <- prioritize_wards(data = combined_wards2, 
                                         population_col = "Population", 
                                         rank_col = "ranks", 
...
combined_plot2 <- katsina_shp %>% 
  ...
  mutate(status = ifelse(is.na(WardPopulation), "Not Reprioritized", "Reprioritized")) %>% filter(status == "Reprioritized") %>%  
  left_join()
  
#make datasets for plotting 50 and 75% urban 
ranks <- combined_wards2 %>%  dplyr::select(WardName, median_rank, ranks, Population)
priorite_50 <- prioritized_katsina2 %>%  left_join(ranks, by = c("SelectedWards" = "WardName"))
combined_plot50 <- katsina_shp %>% 
  left_join(priorite_50, by = c("WardName" = "SelectedWards")) %>% mutate(new_rank = rank(ranks)) %>%  
  filter(!is.na(WardPopulation))


priorite_75 <- prioritized_katsina3 %>%  left_join(ranks, by = c("SelectedWards" = "WardName"))
combined_plot75 <- katsina_shp %>% 
  left_join(priorite_75, by = c("WardName" = "SelectedWards")) %>% mutate(new_rank = rank(ranks)) %>%  
  filter(!is.na(WardPopulation))

#plot 50 and 75% urban 
#50
p_50<- ggplot(data = filter(lGAshp, State == "Katsina")) +
  geom_sf(fill = "red") +
  geom_sf(data = combined_plot50, aes(geometry = geometry), fill = "green") +
  # scale_fill_manual(values = c("Not Reprioritized" = "red", "Reprioritized" = "green"),
  #                   name = "Status") +
  geom_text_repel(data = filter(lGAshp, State == "Katsina"),
...
p3_ward = ggplot()+
  geom_sf(data = combined_plot50, aes(geometry = geometry), fill = "green")+
  geom_text_repel(data = combined_plot50,
...
#75
p_75<- ggplot(data = filter(lGAshp, State == "Katsina")) +
  geom_sf(fill = "red") +
  geom_sf(data = combined_plot75, aes(geometry = geometry), fill = "green") +
  # scale_fill_manual(values = c("Not Reprioritized" = "red", "Reprioritized" = "green"),
  #                   name = "Status") +
  geom_text_repel(data = filter(lGAshp, State == "Katsina"),
                  aes(label =  LGA, geometry = geometry),color ='black',
...
p4_ward = ggplot()+
  geom_sf(data = combined_plot75, aes(geometry = geometry), fill = "green")+
  geom_text_repel(data = combined_plot75,
                  aes(label =  paste0(new_rank,". ", WardName, " (", round(WardPopulation*1.1, 0), ")"), geometry = geometry),color ='black',
                  stat = "sf_coordinates", min.segment.length = 0, size = 3.5, force = 1, max.overlaps = Inf)+
  map_theme() +
  xlab("")+
...
ggsave(paste0(FigDir,"/", "urban_50_75", "/", Sys.Date(),"_Katsina_reprioritization_scenarios.pdf"), p_high_urban, width = 13, height = 14)
