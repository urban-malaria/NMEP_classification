...
Kano_lGAshp <- sf::st_read(file.path(pathLGA, "NGA_LGAs.shp"))
sf::sf_use_s2(FALSE)

lga_ward <- sf::st_intersection( Kano_shp, Kano_lGAshp)
...
combined_wards <- left_join(Kano_variables, Kano_ranks , by = c("WardCode" = "WardCode"))

combined_wards2 <- left_join(combined_wards, Kano_itn_clean, by = c("WardName.x" = "Ward")) 
